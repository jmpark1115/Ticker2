...
bithumbkey    = '9b7b5dabf5961351cfd1fefd56e2b58e'
bithumbsecret = '48781d1e5a02d03bee5a7e50df8d8405'
...
res = bithumb.Balance()
target = float(res['data']['available_' + target.lower()])
base   = float(res['data']['available_' + payment.lower()])

#orderbook
res = bithumb.Orderbook()
asks_price = float(res['data']['asks'][0]['price'])
asks_qty = float(res['data']['asks'][0]['quantity'])
bids_price = float(res['data']['bids'][0]['price'])
bids_qty = float(res['data']['bids'][0]['quantity'])

orderQty = 12
orderSide='SELL'
res = bithumb.Order(price=bids_price, qty=orderQty, side=orderSide)
order_id = res['order_id']
print(f'order_id {order_id} res {res}')

res= bithumb.Order_info(order_id=order_id, qty=orderQty, side=orderSide)
print(res)

# Cancel(self, order_id, price=0, side=None, qty=0):
res = bithumb.Cancel(order_id=order_id, price=bids_price, side=orderSide, qty=orderQty)
print(res)
